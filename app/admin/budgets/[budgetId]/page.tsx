...
export async function generateMetadata({
  ...
  return {
    ...
    description: `Edita tu presupuesto: ${budget.name}`,
  };
}
...
export default async function BudgetDetailsPage({
  ...
  const percetage = +((totalSpent / +budget.amount) * 100).toFixed(2);
      ...
      {budget.expenses.length ? (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2  mt-10">
            <div>
              <ProgressBar percentage={percetage} />
            </div>
          ...
          <ul
            role="list"
            className="divide-y divide-gray-300 border shadow-lg mt-10 rounded-lg"
          >
        ...
        <p className="text-center py-20 text-lg font-bold">No hay gastos aún</p>
      )}
